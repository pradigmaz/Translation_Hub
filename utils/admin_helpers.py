...
# ImageContent, ProjectDocument удалены в content/migrations/0003
...
class FileSystemAdminHelpers:
        ...
        def build_node(path):
            ...
            if path.is_file():
                try:
                    ...
                    node["modified"] = timezone.datetime.fromtimestamp(
                        path.stat().st_mtime
                    )
                except:
                    pass
    ...
    def get_structure_statistics():
        ...
        # Проверяем пользователей (оптимизированный запрос)
        ...
        # Проверяем команды (оптимизированный запрос)
        for team in Team.objects.only("id", "name"):
        ...
        # Проверяем проекты (оптимизированный запрос)
        for project in Project.objects.select_related("team").only(
            "id", "title", "content_folder", "team__id", "team__name"
        ):
            project_path = FilePathManager.get_project_path(
                project.team.id, project.content_folder
            )
    ...
    def get_general_file_statistics():
        ...
        stats = {
            "total_images": 0,  # ImageContent удалена
            "total_documents": 0,  # ProjectDocument удалена
            "total_file_size": 0,
            "avg_file_size": 0,
            "file_types": {},
            "upload_trends": [],
        ...
        image_stats = {  # ImageContent удалена
            "total_size": 0,
            "avg_size": 0,
        }

        doc_types_old = ProjectDocument.objects.values("document_type").annotate(
    ...
    def get_user_file_statistics():
        """Статистика файлов по пользователям."""
        user_stats = []

        # Оптимизированный запрос с аннотациями
        users_with_stats = User.objects.annotate(
            images_count=Count("imagecontent", distinct=True),
            documents_count=Count("projectdocument", distinct=True),
        ).only("id", "username", "avatar")[:50]
        ...
        for user in users_with_stats:
            ...
            stats = {
                ...
                "documents_uploaded": user.documents_count,
            }

            if user_path.exists():
                try:
                    total_size, file_count = FileSystemAdminHelpers._calculate_directory_size(user_path)
    ...
    def get_team_file_statistics():
        ...
        for team in teams_with_stats:
            ...
            if team_path.exists():
                try:
                    total_size, file_count = FileSystemAdminHelpers._calculate_directory_size(team_path)
                    stats["directory_size"] = total_size
                    stats["file_count"] = file_count
                except Exception as e:
                    stats["error"] = str(e)
    ...
    def get_project_file_statistics():
        ...
        # Оптимизированный запрос с аннотациями и select_related
        projects_with_stats = (
            ...
            .only("id", "title", "status", "content_folder", "team__id", "team__name")[
                :100
            ]
        )

        for project in projects_with_stats:
            project_path = FilePathManager.get_project_path(
                project.team.id, project.content_folder
            )
            ...
            if project_path.exists():
                try:
                    total_size, file_count = FileSystemAdminHelpers._calculate_directory_size(project_path)
                    stats["directory_size"] = total_size
                    stats["file_count"] = file_count
                except Exception as e:
                    stats["error"] = str(e)
    ...
    def get_large_files(limit=20):
        ...
        try:
            ...
            large_files = sorted(all_files, key=lambda x: x["size"], reverse=True)[
                :limit
            ]
    ...
    def check_structure_integrity():
        ...
        for project in Project.objects.select_related("team"):
            ...
            if not project_path.exists():
                has_files = (
                    ImageContent.objects.filter(project=project).exists()
                    or ProjectDocument.objects.filter(project=project).exists()
                )
                        ...
                        for file_path in user_dir.rglob("*"):
                            if file_path.is_file():
                                ...
                                if not user_exists:
                                    ...
                                    orphaned["summary"]["total_size"] += file_info[
                                        "size"
                                    ]
    ...
    def find_duplicate_files():
        ...
        duplicates = {
            "duplicate_groups": [],
            "summary": {"total_duplicates": 0, "wasted_space": 0},
        }
        ...
        try:
            ...
            for file_hash, files in file_hashes.items():
                if len(files) > 1:
                    ...
                    duplicates["summary"]["wasted_space"] += files[0]["size"] * (
                        len(files) - 1
                    )
        ...
        try:
            ...
            for project in Project.objects.select_related("team"):
                ...
                if has_files:
                    ...
                    if not project_path.exists():
                        DirectoryManager.create_project_directory(
                            project.team.id, project.content_folder
                        )
                        result["directories_created"] += 1
    ...
    def create_missing_directories():
        ...
        result = {"success": True, "directories_created": 0, "errors": []}
        ...
        try:
            ...
            for missing_dir in integrity_report["missing_directories"]:
                try:
                    ...
                    result["errors"].append(
                        {"path": missing_dir["path"], "error": str(e)}
                    )
